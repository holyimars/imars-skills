...
2. For any Java interface/impl question: if only codegraph is available, cross-check `cg-node.sh` on the interface method AND the impl method (or a single `cg-explore.sh` call) before stating a caller count or dead-code verdict. If only codebase-memory-mcp is available, the interface-method union query is MANDATORY, not optional, before any such verdict. If both are available, follow the skill's decision table.
