...
**0.0.28 进一步扩大覆盖面**:Bash 里跑的 `find`(`-name`/`-iname`/`-path`/`-wholename`/`-ipath`/`-iwholename` 携带的取值,`-regex`/`-iregex` 故意不算——取值是正则而非 shell glob,修剪逻辑对它语义不对,排除后干净不触发)和**带具体查找模式**的 `ls`/`tree`(`tree -P` 参数,或者 `ls`/`tree` 后面跟着一个含 `*`/`?` 的位置参数,比如 `ls src/*Controller*`)现在也会被识别,复用同一套查询/合并/注入逻辑。`ls`/`tree` 的**裸调用**(`ls src/`、`tree`、`tree -L 2` 这种纯目录浏览、没有任何模式)刻意不触发——grep/find 分支靠"查得到就转符号、查不到就干净退出"天然兜底噪音,但裸调用没有可提取的 pattern,强行触发只会变成对着每一次目录浏览都弹提示,而 `ls`/`tree` 的调用频率远高于 grep,会实质性抬高噪音,与这个 hook"非阻断、best-effort、低噪音"的设计原则冲突,是刻意的产品决策而非遗漏。

**0.0.29 给 find 分支加了一层"意图 guard"**:上线后针对精度做的对抗测试发现,find 分支会对清理/维护类命令(`find . -name "*.log" -mtime +30 -delete`、`find ... -exec rm {} \;`、`find ... | xargs rm`)和 `-prune` 排除语义(`find . -iname "node_modules" -prune -o -print`——`node_modules` 在这里是被排除的目录,不是查找目标,而且是极常见的真实名字,查询很可能返回文不对题但看似真实的结果,比"干净查空"更差)一样触发注入,这两类当时被记成"已接受的低成本假阳性",这一版收紧掉了。做法:利用 `FIND_RE` 本身贪婪匹配"总是取子句里最后一个 name 系 flag"这个性质,从同一次正则匹配结果(`BASH_REMATCH[0]`)切出"匹配之后、同子句剩余的部分"(含其后的管道段,专为识别 `| xargs rm` 放开),检查这段文本里是否出现 `-delete`、`-exec`/`-execdir`/`-ok`/`-okdir` 调用 `rm`/`rmdir`/`unlink`/`shred`、`| xargs ... rm` 系管道、或 `-prune`——命中就放弃这次提取,不触发。这个判断**只在同一次匹配切出的同子句范围内生效**,不是另开一条扫全字符串的独立正则,不会重蹈"两条正则各自锚定到不同子句"的坑。"取到的一定是子句里最后一个 name flag"是保守方向的近似而非严格证明(引号内含 `;`/`&`/`|` 的取值会打断贪婪扫描,`-prune` 也可能是在修饰更后面的非 `-name` 条件而非本次取到的候选值),但两种失效情形都只会让本该触发的边缘用例多静默几个,不会导致该拦的没拦住。已知仍未覆盖:`mv`(归档/清理语义暧昧,未纳入)、`-exec sudo rm`/`-exec sh -c 'rm ...'`/`-exec busybox rm` 等非直接调用 `rm` 的间接删除写法、`ls --ignore=PATTERN` 这类 ls/tree 侧的排除语义(现状是提取质量差而非语义反转,和这次的问题不是同一类)。

**0.0.30 修了 ROOT 一直锁死在"当前会话自己项目"这个问题**:用真实仓库(`/d/data/RuoYi-Vue-Plus`,已建好 `.codegraph`)做端到端测试发现,从别的会话里用 Bash 工具直接 `grep -rln "AuthController" /d/data/RuoYi-Vue-Plus --include=*.java`(`AuthController` 在那个仓库里真实存在、索引里也查得到),hook 完全没有注入任何提示——`bash -x` 追踪确认 `PATTERN` 提取完全正确,但 `ROOT=$(git rev-parse --show-toplevel)` 只会算出 hook 进程自己继承到的 cwd(当前会话自己的项目目录),命令参数里写的路径它压根看不见,于是查了一个没建索引、也没在 cbm 里注册的错误仓库,静默退出。修法是两层,按信任度从高到低依次尝试:第一层读 PreToolUse hook JSON 本身就带的顶层 `cwd` 字段(Claude Code 官方文档确认这个字段会跟随会话内的 `cd` 漂移,不是固定的会话启动目录)——零正则改动,对 Grep/Glob/grep/rg/ag/find/ls/tree 全部分支同时生效,修好"先 `cd` 进目标仓库再检索"这种模式;第二层从命令/工具调用本身抽取一个显式目标目录(`SEARCH_DIR`)——Grep/Glob 直接读 `tool_input.path`,`find` 分支给 `FIND_RE` 加一个新的可选捕获组抓"find 之后、`-name` 系 flag 之前的第一个非 flag token"(跳过 `-H`/`-L`/`-P` 全局 flag),`ls`/`tree` 分支从已经捕获到的 token 里用纯 parameter expansion 切出目录部分——只做 find/ls/tree/Grep/Glob,`grep`/`rg`/`ag` 这次没做(它们的路径参数既可能在 flag 前也可能在 flag 后,想稳妥处理需要数组+循环从右往左跳过尾部 flag,比这个脚本其余部分依赖的纯 parameter-expansion 技巧复杂一层,留到下一轮单独评估)。这个设计经过独立对抗审查(Fable 5),落地了四条安全约束:`SEARCH_DIR` 只在是绝对路径时才采信(相对路径按 hook 自己的 cwd 解析,不是按会漂移的 shell cwd,不限定的话可能撞上一个真实存在但完全不相关的仓库);`SEARCH_DIR` 解析出的 ROOT,cbm 侧项目名匹配只走精确匹配,不走现有的 `endswith` 模糊兜底(模糊兜底是为"当前会话自己的项目"这个语境设计的,搬到"从命令文本猜出来的目标目录"上会有真实存在但文不对题的项目被误命中的风险,和上一轮 `-prune`/`node_modules` 假阳性同一个等级);新增的 `git -C ... rev-parse` 调用套 `$TMO` 保持超时纪律;`SEARCH_DIR` 是绝对路径但解析失败(不是 git 仓库/目录不存在)时直接 `exit 0`,不再退回查当前会话自己的仓库(命令已经明确说了要查哪里,退回查别的仓库只是把"查错仓库"的噪音重新引入一遍)。审查还确认了两条不变式,解释了为什么 `find` 分支新增的可选捕获组不会重蹈 0.0.28 ls/tree 分支"贪婪前缀吞掉可选捕获组"的坑:新增组能匹配的字符集是原有贪婪扫描本来就能吞掉的子集(整段匹配、取值组在任何引擎上都不变),且新增组要么完整捕获要么完全不参与(不存在半个 token 或抓错 token 的中间态)——跨引擎的行为差异最坏只是"这次不生效",不会是"抓错"。实测 9 种 find 命令形状 + 4 种 ls/tree 形状 + 完整重跑 0.0.29 的 31 个回归用例,零回归;另外用真实 Bash 工具(不 `cd`,直接对 `/d/data/RuoYi-Vue-Plus` 跑 `find`)和合成 `cwd` 字段两种方式验证了端到端效果,确认了两层跟随、约束 2/4 各自的边界行为。`grep`/`rg`/`ag` 分支目前的应对方式记在 `skills/code-navigator/SKILL.md` 里:查一个不同的已索引仓库,优先 `cd` 进去再用相对路径检索。

**0.0.31 是对 0.0.28–0.0.30 三轮改动的一次结构化 code review(`/code-review-expert`)加修复**:审查按 SOLID/安全/质量/边界四个维度过了一遍未提交 diff,关键疑点全部先在真实脚本上用 `bash -x` 实测复现才立案,共 6 条(P1×1、P2×2、P3×3),用户确认后全部修复。**P1(实测复现的回归)**:文件→父目录裁剪只认 `/`,反斜杠形式的 Windows 文件路径(信任判定明确采信 `D:\...` 形式)裁剪失败,未裁剪的**文件**被喂给 `git -C`,失败后走"显式目标不可解析即 `exit 0`"的约束 4,连 `HOOK_CWD` 兜底也被跳过——对"当前仓库内文件的 Grep(path=反斜杠)"这种 0.0.29 之前能正常增强的调用是静默回归;改成按 `[/\\]` 任意分隔符裁剪,顺带修正混合分隔符路径裁错层级的问题。**P2×2**:含通配符的 SEARCH_DIR(`ls /repo/**/*X*` 切出的 `/repo/**`)永远不可能被 `git -C` 解析,整个形状族静默死亡——新增集中式通配符截断(取最长字面前缀、按分隔符退到完整祖先组件,无分隔符则弃);`ROOT_DYNAMIC` 的"cbm 只走精确匹配"规则在动态解析目标就是会话自己仓库时过严(Grep path 指向当前仓库子目录这种最常见形态会白白关掉模糊兜底)——新增与 SESSION_ROOT 的字符串相等比较,相等即清除标志(git for Windows 对 `/d/...`、`D:/...`、`D:\...`、含 `..`、任意大小写的输入全部归一输出磁盘真实大小写的 `D:/...` 形式,已逐一实测;不等方向的失效只是模糊保持关闭,方向保守)。**P3×3**:相对路径 SEARCH_DIR 不再一律丢弃,`HOOK_CWD` 非空时拼接解析(`find ../other-repo` 能正确跨仓库),加 `[ -e ]` 存在性门控挡住 find 正则可能捕获的 `!`/`\(` 等非路径 token;UNC 路径文档化为刻意边界;三个 Bash 提取分支收敛为函数(`extract_bash_grep`/`find`/`lstree`,逻辑逐字节一致)。

**0.0.31 的修复自身又过了一轮 Fable 5 独立对抗审查,再抓出 4 个真缺陷 + 2 个卫生问题,全部落地**:(1)相对拼接在命令自带 `cd` 时锚错——hook 在命令执行**之前**触发,`HOOK_CWD` 是 cd 之前的目录,`cd src && find .. -name ...` 会拼到错误的父目录,且存在性门控挡不住(拼错的路径往往真实存在,只是不是命令要搜的地方,最坏解析成真实但错误的仓库)——现在命令行里任何命令边界上的 `cd`/`pushd`/`popd` 都会禁用本次相对拼接(方向保守:最多损失新增覆盖,拼接本身绝不**新增**解析错仓库的路径;被禁用后该次调用退回会话仓库层,若命令 `cd` 进的是另一个仓库,这层退回查到的仍是会话自己的仓库——0.0.30 之前就存在的现状噪音,见已知边界与 SKILL.md 的操作建议,0.0.32 起如实限定这句表述;绝对路径不受影响);(2)文件裁剪可能产出裸盘符 `D:`,而 `git -C "D:"` 在 Windows 上是**盘相对**语义,会解析到 hook 进程在该盘的 cwd——真实但错误的仓库(实测证实)——现在裸盘符归一成 `D:/` 让它诚实地失败或成功;(3)SESSION_ROOT 的 legacy 兜底调用漏了 `$TMO`(顺带给 0.0.27 遗留的无超时 legacy ROOT 兜底也补上,TMO 自 0.0.30 起已提前计算,不再有顺序障碍);(4)正斜杠 UNC(`//server/share`——Git Bash 用户实际会打的拼写,反斜杠会被 bash 吃掉)穿过 `/*` 绝对路径分支,产生实测约 2 秒的真实网络探测——现在和反斜杠拼写一样丢弃(`///...` 三斜杠不算 UNC,POSIX 语义等价单斜杠,仍走绝对分支)。卫生问题:所有解析用 git 调用前缀 `env -u GIT_DIR -u GIT_WORK_TREE`(导出的 `GIT_DIR` 会整体压过 `-C`,让所有解析坍缩到同一个仓库,实测证实);`LSTREE_DIR` 显式初始化(它是唯一条件赋值的共享全局,环境导出的同名变量可泄漏进 SEARCH_DIR)。同轮审查确认不成立的担忧和一个如实记录不修的旧账(见下面已知边界最后一条)也都写进了 CHANGELOG。验证:31 个 0.0.29 PATTERN 回归 + 11 个 SEARCH_DIR 净化断言 + 11 个带 `cwd` 的端到端 ROOT/ROOT_DYNAMIC 断言 + 9 个对抗审查场景断言(含 `GIT_DIR` 导出、环境变量泄漏两个需要污染环境的用例),全部通过、零偏差。

**0.0.32 对 0.0.28–0.0.31 全部四轮改动做了一次双轴 `/code-review xhigh` 审查,两轴全部发现均已落地**:Standards 轴(对照本仓库成文不变式 + Fowler 坏味道基线)零硬违规——非阻断、保守失败方向、超时纪律、bash 3.2 regex-in-variable 逐条核验通过——5 条判断题全部处理:三组逐字重复的代码形状(引号剥离×4 处、basename/通配/质量守卫尾部×2 处、裸质量守卫×3 处、会话根解析阶梯×2 处)收敛为共享函数 `strip_quotes`/`pattern_ok`/`qualify_glob_candidate`/`resolve_session_root`(约定经由固定共享全局传值,bash 3.2 无 nameref;并立下"这些 helper 永不使用 `[[ =~ ]]`"的硬规则,因为 find 分支在调用它们之后才用 `BASH_REMATCH[0]` 计算 TAIL);四条识别正则重复的子句边界前缀收敛为单一 `BOUNDARY_RE` 定义——"多行命令第二行不识别"这条旧账的边界字符类从此只存在于一处,将来要修只改一处再重跑矩阵,不再是"同时动三条正则";`GITRP` 重命名为自解释的 `GIT_CLEANENV`;超时预算模型在 TMO 计算处显式写明:`$TMO` 只限定**单次**外部调用 2 秒、不限定总和,最坏情况刻意超出 hook 级 `timeout: 5`——本地 git/jq 调用是毫秒级,只有网络挂载才会吃满单次配额,外层超时杀掉的代价只是丢一次增强(绝不阻塞工具调用),而把 hook 超时抬到理论总和只会让病态调用拖住用户命令更久,是有意的超售而非疏漏;CHANGELOG 0.0.28–0.0.31 四条中文条目译回英文,恢复该文件自 0.0.1 起全英文起笔的内部惯例(README 本身是中文文档,不受影响)。Spec 轴 4 条全部落地:`CD_RE` 改为对 `"$CMD "`(哨兵空格,与 `INTENT_RE` 同一套避开 `$` 锚的引擎可移植性习惯)匹配,命令**末尾**的裸 `cd`/`pushd`/`popd` 现在也会置位禁用标志,"任何命令边界上的 cd 都禁用"从略强于实现变成字面为真;"绝不解析错仓库"按实际语义限定(见上段);SKILL.md 的 cd-first 建议补上触发时序限定(`cd` 必须是**前一次独立** Bash 调用,单条 `cd X && grep ...` 里 hook 读到的还是 cd 之前的目录);脚本头注释补全 0.0.30 四条安全约束的逐条列举;已知边界里的 `fd` 一词是 0.0.28 顺手加入但当时 CHANGELOG 未记,已在 0.0.32 条目补记。验证:全量矩阵(31 个 PATTERN 回归 + 11 个 SEARCH_DIR 净化 + 11 个端到端 ROOT/ROOT_DYNAMIC + 9 个对抗场景)+ 新增 5 个尾部/非边界 cd 用例共 12 条断言,全部通过、零偏差。
...
- 同一条命令子句里出现多个 `-name`/`-iname`(如 `-o` 连接的 OR)或多个含通配符的位置参数(如 `ls src/*.py test/*.ts`)时,贪婪匹配取到的是**最后一个**而不是第一个,是有意接受的已知偏差,和上一条同一档成本;
- `find -name "*.py"`、`tree -P "*.vue"` 这类纯扩展名模式,去掉通配符后会被裁成 `.py`/`.vue` 这种大概率查不到任何符号的短字符串,同样是"查不到就是干净空结果"的接受成本,不是错误;
- UNC 网络路径两种拼写(`\\server\share` 和 `//server/share`)都不采信为检索目标目录(0.0.31)——指向网络挂载的解析意味着每次调用最多 2 秒的探测停顿,而这个形状没有已知真实用途,丢弃后退回 `HOOK_CWD` 层,是刻意边界不是疏漏;
- 单条命令行内 `cd`/`pushd` 进另一个仓库再检索(`cd /other-repo && grep X .`)不会被跟随——hook 在命令执行**前**触发,读到的 `cwd` 是 cd 之前的值,同时 0.0.31 的 cd guard 会禁用该次相对拼接,这次调用退回按会话仓库查询(与 `grep`/`rg`/`ag` 跨仓库现状同级、0.0.30 之前就存在的噪音,不是新引入);跨仓库检索请把 `cd` 作为单独的前一次 Bash 调用(SKILL.md 有同样的操作建议),从下一次调用起 `cwd` 字段就会跟上;
- 多行复合命令里**第二行起**的检索命令完全不识别(`echo hi` 换行 `find . -name ...` 不会触发)——子句边界字符类只有 `;`/`&`/`|` 没有换行,是 0.0.28 起就存在的旧账,0.0.31 的对抗审查将其显式化;修它的收益(多行复合命令里非首行的检索)不及风险,如实记录不修;0.0.32 起该边界字符类收敛为脚本内单一 `BOUNDARY_RE` 定义,将来若要修只需改一处并重跑全套矩阵。
