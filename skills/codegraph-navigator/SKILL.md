...
5. **Freshness.**
   ...
   Never run `codegraph index --force` (full rebuild) while another `codegraph` command might still be in flight in the same session — field-verified 2026-07-18 on Windows: a concurrent read-only `query` alone is enough to make `index` hard-fail with `EPERM: database file is in use` (it aborts cleanly, no corruption, but no retry either). `codegraph sync` was not observed to have this problem and is the correct default for routine freshness.
...
| Compound / exploratory SINGLE-AREA question ("how does X reach Y", "why does Z happen"), English or mixed-language | `scripts/cg-explore.sh "<question...>"` | **flagship tool for this shape only** — one call returns relevant symbols' source + call paths + blast radius, including interface/impl synthesis; prefer this over chaining find→trace→node for anything non-trivial. Do NOT use for whole-graph aggregate questions, see the row below |
| Same shape, but the question is pure Chinese business language(纯中文业务问题,不含任何英文标识符) | `scripts/cg-find.sh '<中文关键词>'` | field-verified 2026-07-18: `cg-explore.sh` returns "No relevant code found" on pure-Chinese queries even when the exact same term hits correctly via `cg-find.sh`'s underlying FTS search — it needs at least one Latin/identifier-shaped token to anchor on. A query mixing one English identifier with Chinese words (e.g. `"SysUser 登录"`) DOES work with `cg-explore.sh`; a pure-Chinese phrase does not |
...
- **`SpringUtils.getBean(X.class).method(...)` calls are invisible to `callers`/`node`/`impact` even though the target is a single, statically-determinable bean — field-verified 2026-07-18, worse than the fan-out case above because nothing is surfaced at all.**
  Confirmed 0/2 recall on real call sites (`DictPatternValidator.java:51`, `ExcelDictConvert.java:65`) reaching `DictService.getDictLabel` this way; the gap propagates directly into `cg-impact.sh`'s blast radius, so a "safe to change" read from impact alone can be wrong.
  Before reporting a final caller/impact count for any interface method, grep `SpringUtils.getBean(<Interface>.class)` for that interface as a standing spot-check — see `references/blindspots.md` for the full repro.
- **Class `extends` relationships are one-directional in the graph — querying a subclass never surfaces its parent (field-verified 2026-07-18, matches upstream open issue #1328).**
  `codegraph node <ParentClass>` lists subclasses under a `Called by ←` heading (mislabeled — it's inheritance, not a call); `codegraph node <ChildClass>` shows nothing about its parent at all, and the node JSON has no `extends`/`superclass` field.
  "What does class X inherit from" needs a grep of the `class X extends Y` declaration, not a graph query.
